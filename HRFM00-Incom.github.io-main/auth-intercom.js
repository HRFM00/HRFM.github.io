// Login + Intercom (WebRTC) only
// Dependencies: supabase-client.js, user-auth-utils.js
...
function showStep(step) {
  ...
  const showRooms = step === 'rooms' || step === 'inroom';
  elStepRooms.classList.toggle('hidden', !showRooms);
...
function updateParticipantsUI() {
  ...
  for (const key in state) {
    for (const meta of state[key]) entries.push(meta);
  }
  ...
  entries.forEach(meta => {
    ...
    if (speakingState.get(meta.clientId)) avatar.classList.add('speaking');
  });
  updateCounts();
}
...
function ensurePeerConnection(peerId, peerName) {
  ...
  pc.onicecandidate = (ev) => {
    if (ev.candidate) sendSignal('webrtc:candidate', { targetId: peerId, candidate: ev.candidate });
  };
  ...
  pc.onconnectionstatechange = () => {
    ...
    if (st === 'failed' || st === 'disconnected' || st === 'closed') closePeer(peerId);
  };
...
async function handleOffer(fromId, payload) {
  ...
  if (localStream && pc.getSenders().length === 0) for (const track of localStream.getTracks()) pc.addTrack(track, localStream);
...
async function joinRoom(room) {
  ...
  roomChannel
    .on('presence', { event: 'sync' }, () => {
      ...
      const others = [];
      for (const key in state) for (const meta of state[key]) if (meta.clientId !== clientId) others.push(meta);
      others.sort((a, b) => a.clientId.localeCompare(b.clientId));
      for (const meta of others) if (clientId < meta.clientId) startOffer(meta.clientId, meta.userName).catch(err => log('offer error', err));
    })
    ...
    .on('presence', { event: 'leave' }, ({ leftPresences }) => {
      for (const p of leftPresences) { log(`leave ${p.userName}`); closePeer(p.clientId); }
      updateParticipantsUI();
    })
    ...
    .on('broadcast', { event: 'webrtc:leave' }, ({ payload }) => { closePeer(payload.fromId); });
...
async function leaveRoom() {
  ...
  isMuted = false;
  if (btnMute) btnMute.textContent = 'ミュート';
  currentRoom = '';
...
if (btnLogin) {
  btnLogin.addEventListener('click', async () => {
    ...
    try {
      ...
      if (!result.success) { alert(result.error || 'ログインに失敗しました'); return; }
      window.userAuthUtils.saveSession(result.user);
      userName = getUserDisplayName(result.user);
      showStep('rooms');
    } catch (e) { alert('ログインエラー: ' + (e?.message || e)); }
  });
}
...
if (btnSaveConfig) {
  btnSaveConfig.addEventListener('click', () => {
    ...
    if (!authUrl || !authKey || !commsUrl || !commsKey) { alert('すべての設定を入力してください'); return; }
...
joinButtons.forEach(btn => {
  btn.addEventListener('click', () => {
    ...
    joinRoom(room).catch(err => { log('joinRoom error', err); alert('入室に失敗しました。設定を確認してください。'); });
  });
});
